#!/usr/bin/env bash
# restart-tether.sh — run the CURRENT committed code cleanly.
...
nohup node agent-backend/server.js                         > /tmp/tether-backend.log  2>&1 &
BE=$!
nohup npm --prefix dashboard run start                     > /tmp/tether-frontend.log 2>&1 &
FE=$!
...
echo "✓ backend pid $BE (log: /tmp/tether-backend.log)"
echo "✓ frontend pid $FE (log: /tmp/tether-frontend.log)"
echo "✓ open http://localhost:6801"
echo "  (per-session files live under ~/.tether/sessions/<id>/workspace)"
