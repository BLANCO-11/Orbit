...
└──────────────────────┘                                 └────────────┬──────────────┘
   ...
   preview · policies · fleet                                          ▼
                                              ...
                                              └───────┬───────────────────────┬────────┘
                                                      │ tools                 │ MCP (stdio)
                                                      ▼                       ▼
                                     per-session workspace          tether MCP servers:
                                     ~/.tether/sessions/<id>/         lightpanda (browser),
...
| **PostgreSQL** *(optional)* | the default DB in Docker; set `DATABASE_URL` | omit it and Tether uses local SQLite with no config |
...
```bash
git clone https://github.com/BLANCO-11/Tether.git
cd Tether
npm install
npm --prefix dashboard install

cp .env.example .env          # set LLM_BASE_URL / LLM_API_KEY /
                              #     LLM_FAST_MODEL / LLM_REASONING_MODEL
                              # (these become the onboarding defaults; or leave
                              #  blank and configure in Settings after launch)

npm run dev                   # backend :6800 + dashboard :6801
# open http://localhost:6801
```

On first boot Tether **seeds `agent-backend/security-config.json`** from the committed example (so it doesn't crash on a fresh clone) and **auto-starts the Lightpanda browser** container if Docker is available.
...
**Database** — Tether runs on either **PostgreSQL** or **SQLite**, chosen at boot: set `DB_DRIVER` explicitly, else a `DATABASE_URL` selects Postgres, else it falls back to SQLite (`node:sqlite`) at `DB_PATH`. SQLite is the zero-config default for local/single-box; the Docker stack ships a Postgres service and defaults to it. Migrate an existing SQLite DB with `agent-backend/scripts/migrate-sqlite-to-pg.js` (see `.env.example`). Both drivers share one schema and the same app behavior.

**Permissions** are a capability × mode matrix (`chat`/`plan`/`edit`/`yolo` → allow/ask/block) enforced by the backend on every tool call, plus a consent-proof hard blocklist (your `~/.ssh`, Tether's own source, etc.). Shell commands are tokenized so a blocklisted path can't slip through via redirects/subshells/unlisted tools. Each session writes only inside its own `~/.tether/sessions/<id>/workspace`; anything outside asks first. For untrusted execution, set `SANDBOX_DEFAULT=container` to run agents in an ephemeral Docker sandbox by default. The container sandbox ships a python+node image (`SANDBOX_IMAGE`, pulled on first use — set `SANDBOX_PULL=never` for air-gapped hosts) and its network is on by default (`SANDBOX_NETWORK`). Run-API executions are bounded by an idle watchdog and an absolute backstop (`RUN_IDLE_MS`, `RUN_MAX_MS`) so a hung task always terminates.
...
- **Mission board** — the agent's structured plan (via the `tether-plan` tool) as a live checklist with dependencies.
...
agent-backend/        Express backend, harnesses, policy engine, MCP registry
  ...
  workspace-paths.js  per-session ~/.tether/sessions/<id> layout
...
restart-tether.sh      clean rebuild + boot
```
...
DATABASE_URL=postgres://tether:tether@localhost:5432/tether_test node tests/test_db_layer.js
```
...
- **PostgreSQL** is the default DB: the `postgres` service holds the data (on the
  `tether-pgdata` volume) and the app waits for it via a healthcheck. To use SQLite
  instead, comment out `DATABASE_URL` on the `tether` service (or set
  `DB_DRIVER=sqlite`); it then persists at `DB_PATH` on `tether-data`.
  Override `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB` in `.env` for a real
  deploy (and keep `DATABASE_URL` in sync).
- Session workspaces persist on the `tether-data` volume (`APP_HOME`).
...
Any machine can join Tether as a harness. In **Fleet → Pair a device** you get a
...
(`agent-backend/adapter/tether-connect.js`) pairs, then Tether drives that machine
exactly like the local harness (spawn/prompt/cancel, streamed events, central
policy). Tether supplies the **plan/context**; the remote runs its **own** agent
with its **own** model and tools — Tether never provides inference.

The connector is **agent-agnostic**: it auto-detects an installed agent on `PATH`
and translates its native protocol to Tether's events via a per-agent adapter:
...
| **custom** | `AGENT_CMD` (+ optional `AGENT_ARGS` with `{prompt}`) | text — drive any other agent |
| **generic** | built-in OpenAI-compatible tool loop | forced only (`AGENT_CONNECT_KIND=generic`), needs `LLM_*` |

Force a specific one with `AGENT_CONNECT_KIND=<pi|claude|opencode|codex|gemini|aider|custom|generic>`.
...
`AGENT_CMD` to point at the exact command. The connector probes `pi --help`
...
- **Isolated workspace, on the remote.** Each session runs in
  `~/.tether/workspaces/<sessionId>` on the *remote* box; the local host's sessions
  live in `~/.tether/sessions/<sessionId>/`. Both keyed by session id.
...
- **Operator console → the agent's runtime.** The Console runs your commands where
  the agent runs — the Tether host for a local agent, the remote machine for a
...
- **Tether MCP tools work.** The connector hands the agent `AGENT_API_URL` +
  `AGENT_API_KEY` (the device token) + `AGENT_SESSION_ID`, so the agent's Tether
  MCP tools (fleet/notify) authenticate back to Tether as that device.
...
> public origin on the backend: `APP_PUBLIC_ORIGIN=https://tether.example.com`.
> Tether then always emits `wss://tether.example.com/api/harness` and ignores the
> request headers entirely.

**Verify the harness WS path end-to-end** (expect `HTTP/1.1 401` — the upgrade
reached Tether and it rejected the fake token; a `200`/`404`/`502`/`301` means the
proxy isn't upgrading):

```bash
curl -i -N -H "Connection: Upgrade" -H "Upgrade: websocket" \
  -H "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==" -H "Sec-WebSocket-Version: 13" \
  'https://tether.example.com/api/harness?token=probe'
```
...
> If Tether sits behind **another** TLS terminator (so this nginx listens on plain
...
```yaml
labels:
  - "traefik.http.routers.tether-api.rule=Host(`tether.example.com`) && PathPrefix(`/api`)"
  - "traefik.http.services.tether-api.loadbalancer.server.port=6800"
  - "traefik.http.routers.tether-ui.rule=Host(`tether.example.com`)"
  - "traefik.http.services.tether-ui.loadbalancer.server.port=6801"
```
...
- **Cloudflare Tunnel (`cloudflared`):** WS works, but the origin service is
  ...
  `APP_PUBLIC_ORIGIN=https://tether.example.com` on the backend so the descriptor
  is always `wss://`.

### OpenShift / Kubernetes routes

Routes support WebSockets by default (edge/passthrough TLS). If Tether is behind an
in-cluster nginx too, apply the nginx block above and make sure **both** layers
upgrade WS and preserve the https scheme. When in doubt, set `APP_PUBLIC_ORIGIN`.
...
Never commit `.env`, `agent-backend/security-config.json`, or `agent-backend/.tether-secret` — they're gitignored and hold secrets. `security-config.example.json` is the safe, committed template.

## Tether as a Headless Backend

Tether can be run as a headless backend service (`agent-backend`). This allows third-party developers to connect their own dashboards, voice control suites, or external scripts.

**Parent-app run flow.** With only its Tether API key, an external app can drive an agent end-to-end and read a typed result:
