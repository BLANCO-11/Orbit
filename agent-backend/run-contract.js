...
function listArtifacts(dir, { maxFiles = 500, prefix = "/artifacts" } = {}) {
  ...
  const walk = (abs, rel) => {
    ...
    for (const e of entries) {
      ...
      let bytes = 0;
      let mtimeMs = 0;
      try { const st = fs.statSync(childAbs); bytes = st.size; mtimeMs = st.mtimeMs; } catch {}
      const { language, mime, isCode } = classifyFile(childRel);
      out.push({ path: `${prefix}/${childRel}`, rel: childRel, language, mime, bytes, isCode, mtimeMs });
    }
  };
  walk(dir, "");
  return out;
...
// ── Integrity checks on the agent's self-report ───────────────────────
// The schema validator above only checks SHAPE. These two check whether the
// self-report is internally consistent and current — an agent can emit a
// perfectly-shaped RESULT.json that contradicts itself or that predates its own
// last edit, and without these the contract reports "succeeded" either way.

// Grace window so same-second writes (RESULT.json and a sibling flushed together)
// don't read as "the artifact is newer".
const STALE_GRACE_MS = 1500;

/**
 * Find non-zero exit codes anywhere in the self-reported `tests` object.
 *
 * The script-gen skill lets the agent nest per-category detail
 * (tests.categories.<name>.exit_code), and nothing stopped it reporting
 * `passed: true` beside `exit_code: 1` — which is exactly what happened on the
 * run that motivated this: `dry_run_execution: { passed: true, exit_code: 1 }`
 * propagated all the way to status "succeeded" on a script that exited 1.
 * A non-zero exit code is machine truth and outranks the agent's own boolean.
 */
function findFailingExitCodes(tests, depth = 0) {
  const hits = [];
  if (!tests || typeof tests !== "object" || depth > 4) return hits;
  for (const [k, v] of Object.entries(tests)) {
    if (/^exit_?code$/i.test(k)) {
      const n = Number(v);
      if (Number.isFinite(n) && n !== 0) hits.push({ key: k, value: n });
    } else if (v && typeof v === "object") {
      hits.push(...findFailingExitCodes(v, depth + 1));
    }
  }
  return hits;
}
...
/**
 * Detect a RESULT.json that was written BEFORE the deliverable's final state.
 *
 * RESULT.json is a claim about the artifacts, so it must be the last thing
 * written. On the motivating run the agent wrote RESULT.json, THEN edited the
 * script to fix a dead URL, then stopped without re-running or rewriting the
 * report — leaving a passing test claim describing code that no longer existed.
 *
 * Only DELIVERABLES count. Three kinds of file are ignored, because a change to
 * them says nothing about whether the test claim still holds:
 *   • dot-prefixed paths (`.venv/…`) — toolchain churn;
 *   • compiled output (`__pycache__/…`, `*.pyc`) — regenerated by merely running
 *     the script, so a smoke test would flag its own report;
 *   • the agent's bookkeeping files (STATUS.json) — progress notes it keeps
 *     updating after the report is filed. Observed flagging an otherwise-clean
 *     run six seconds after RESULT.json was written.
 */
const NON_DELIVERABLE = /^(STATUS\.json|.*\.pyc)$/i;
function isDeliverable(rel) {
  const segs = rel.split("/");
  if (segs.some((s) => s.startsWith(".") || s === "__pycache__")) return false;
  return !NON_DELIVERABLE.test(segs[segs.length - 1]);
}

function findStaleResult(artifacts) {
  const rj = artifacts.find((a) => a.rel === "RESULT.json");
  if (!rj || !rj.mtimeMs) return null;
  const newer = artifacts
    .filter((a) => a.rel !== "RESULT.json"
      && isDeliverable(a.rel)
      && a.mtimeMs > rj.mtimeMs + STALE_GRACE_MS)
    .sort((a, b) => b.mtimeMs - a.mtimeMs);
  if (!newer.length) return null;
  return { newestPath: newer[0].path, newerCount: newer.length, driftMs: Math.round(newer[0].mtimeMs - rj.mtimeMs) };
}
...
//   integrity: { failingExits?: [...], stale?: {...} } — self-report cross-checks
//              (see findFailingExitCodes / findStaleResult). Optional so existing
//              two-arg callers keep their behaviour.
function deriveStatus(lifecycle, result, integrity = {}) {
  ...
  // A non-zero exit code in the agent's own report is machine truth — it outranks
  // a claimed pass, so a self-contradicting report can never read as success.
  if ((integrity.failingExits || []).length) return "failed";
  // A report older than the artifacts it describes is unverified, not disproven:
  // surface it for review rather than calling it a failure.
  if (integrity.stale) return "needs_review";
  return "succeeded";
}
...
function assembleContract({ runId, sessionId, seq, lifecycle, finalMessage, usage, error }) {
  ...
  // Cross-check the self-report before trusting it (see the integrity helpers).
  const failingExits = result.valid && result.data ? findFailingExitCodes(result.data.tests) : [];
  const stale = findStaleResult(artifacts);
  const status = deriveStatus(lifecycle, result, { failingExits, stale });
  ...
  if (result.valid && result.data.tests) {
    ...
    tests = {
      // A claimed pass is overridden by a non-zero exit code in the same report —
      // the contract must not repeat a self-contradiction upward.
      ran: !!t.ran, passed: !!t.passed && failingExits.length === 0,
  ...
  return {
    ...
    raw: {
      ...
      // Why a shaped-and-valid report was still not taken at face value.
      ...(failingExits.length ? { resultJsonContradictions: failingExits } : {}),
      ...(stale ? { resultJsonStale: stale } : {}),
    },
  };
}
